...
class Booster {
 public:
  ...
  void SaveModelToFile(int start_iteration, int num_iteration,
    ...
    boosting_->SaveModelToFile(start_iteration, num_iteration, num_labels, num_label, filename);
  }  
...
int LGBM_BoosterSaveModel2(BoosterHandle handle,
  ...
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  ref_booster->SaveModelToFile(start_iteration, num_iteration, num_labels, num_label, filename);
  API_END();
}
