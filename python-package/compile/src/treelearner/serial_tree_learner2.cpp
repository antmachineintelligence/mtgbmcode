...
#include "cost_effective_gradient_boosting2.hpp"
...
SerialTreeLearner2::SerialTreeLearner2(const Config* config)
  :config_(config) {
...
SerialTreeLearner2::~SerialTreeLearner2() {
  #ifdef TIMETAG
  Log::Info("SerialTreeLearner2::init_train costs %f", init_train_time * 1e-3);
  Log::Info("SerialTreeLearner2::init_split costs %f", init_split_time * 1e-3);
  Log::Info("SerialTreeLearner2::hist_build costs %f", hist_time * 1e-3);
  Log::Info("SerialTreeLearner2::find_split costs %f", find_split_time * 1e-3);
  Log::Info("SerialTreeLearner2::split costs %f", split_time * 1e-3);
  Log::Info("SerialTreeLearner2::ordered_bin costs %f", ordered_bin_time * 1e-3);
  #endif
}

void SerialTreeLearner2::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  if (CostEfficientGradientBoosting2::IsEnable(config_)) {
    cegb_.reset(new CostEfficientGradientBoosting2(this));
    cegb_->Init();
  }
}

void SerialTreeLearner2::ResetTrainingData(const Dataset* train_data) {
  train_data_ = train_data;
  num_data_ = train_data_->num_data();
...
void SerialTreeLearner2::ResetConfig(const Config* config) {
...
Tree* SerialTreeLearner2::Train(const score_t* gradients, const score_t *hessians, bool is_constant_hessian, const Json& forced_split_json) {
...
void SerialTreeLearner2::Train_serial2(Tree* tree, const score_t* gradients, const score_t* hessians){
    
      
//       Log::Info("renew %d %d",data_partition_->num_datas(),data_partition_->get_used_data_count_());
      if(data_partition_->num_datas() < data_partition_->get_used_data_count_()){
//           std::vector<size_t> data_isinbag(data_partition_->get_used_data_count_());
          std::vector<size_t> data_index(data_partition_->num_datas());
//           for (int i = 0; i < data_partition_->get_used_data_count_(); ++i) {
//               data_isinbag[i] = 0;
//           }
          int ii = 0;
          for (int i = 0; i < data_partition_->num_datas(); ++i) {
//               data_isinbag[*(data_partition_->get_used_data_indices() + i)] = 1;
              data_index[i] = *(data_partition_->get_used_data_indices() + i);
    //           if (i > 12000){
    //               Log::Info("renew %d %d",*(data_partition_->get_used_data_indices() + i),i);
    //               data_isinbag[*(data_partition_->get_used_data_indices() + i)] = 1;
    //           }
          }    
...
//                   if (data_isinbag[tmp_idx[j]] == 1){
                      sum_gradients += gradients[data_index[tmp_idx[j]]];
                      sum_hessians += hessians[data_index[tmp_idx[j]]];
//                   }    

              }
              tree->SetLeafOutput(i, -sum_gradients / sum_hessians);
          }     
      }else{
           ...
           for (int i = 0; i < tree->num_leaves(); ++i) {
              ...
              for (data_size_t j = 0; j < cnt_leaf_data; ++j) {
    //               if (data_isinbag[tmp_idx[j]] == 1){
                      sum_gradients += gradients[tmp_idx[j]];
                      sum_hessians += hessians[tmp_idx[j]];
    //               }    

    //               Log::Info("renew-- %d %d %d %f",i,j,tmp_idx[j],gradients[tmp_idx[j]]);
              }
    //           Log::Info("renew %d %f %f",i,-sum_gradients / sum_hessians,tree->LeafOutput(i));
              tree->SetLeafOutput(i, -sum_gradients / sum_hessians);
          }         
      }
...
Tree* SerialTreeLearner2::FitByExistingTree(const Tree* old_tree, const score_t* gradients, const score_t *hessians) const {
...
Tree* SerialTreeLearner2::FitByExistingTree(const Tree* old_tree, const std::vector<int>& leaf_pred, const score_t* gradients, const score_t *hessians) {
  data_partition_->ResetByLeafPred(leaf_pred, old_tree->num_leaves());
  return FitByExistingTree(old_tree, gradients, hessians);
}

std::vector<int8_t> SerialTreeLearner2::GetUsedFeatures(bool is_tree_level) {
...
void SerialTreeLearner2::BeforeTrain() {
...
bool SerialTreeLearner2::BeforeFindBestSplit(const Tree* tree, int left_leaf, int right_leaf) {
...
void SerialTreeLearner2::ConstructHistograms(const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  #ifdef TIMETAG
  auto start_time = std::chrono::steady_clock::now();
  #endif
...
void SerialTreeLearner2::FindBestSplitsFromHistograms(const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  #ifdef TIMETAG
  auto start_time = std::chrono::steady_clock::now();
  #endif
...
int32_t SerialTreeLearner2::ForceSplits(Tree* tree, const Json& forced_split_json, int* left_leaf,
...
void SerialTreeLearner2::Split(Tree* tree, int best_leaf, int* left_leaf, int* right_leaf) {
//                   for (int i = 0; i < tree->num_leaves(); ++i) {
//       double output = static_cast<double>(tree->LeafOutput(i));
//           Log::Info("TrainOneIter_a %d %f",i,output);
//     }  
...
void SerialTreeLearner2::RenewTreeOutput(Tree* tree, const ObjectiveFunction* obj, std::function<double(const label_t*, int)> residual_getter,
